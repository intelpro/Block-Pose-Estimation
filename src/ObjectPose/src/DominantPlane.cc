...
namespace Plane {
    ...
    Plane_model DominantPlane::RunRansac(cv::Mat &pcd_inlier) {
       ...
       compute_inlier_cost(best_plane, pcd_inlier, Pointcloud);
    ...
    void DominantPlane::CheckTemporalConsistency(Plane_model& best_plane, float best_cost)
    {
        ...
        {
            error = std::abs(prev_plane.a - best_plane.a) + std::abs(prev_plane.b - best_plane.b) + 
                  std::abs(prev_plane.c - best_plane.c) + std::abs(prev_plane.d - best_plane.d);
            cost = best_cost;
