...
void ObjectPose::Accumulate_PointCloud(cv::Mat &pcd_outlier, cv::Mat &Mask)
{
...
void ObjectPose::ProjectToDominantPlane()
{
    ...
    for(int i = 0; i < merged_cloud.size(); i++)
    {
        pcl::PointXYZRGB point  = merged_cloud[i];
...
void ObjectPose::ProjectedCloudToImagePlane()
{
    ...
    cv::imwrite("projected_image2.png", projected_image);
...
void ObjectPose::fitRectangle(cv::Mat projected_image)
{
   ...
   vector<RotatedRect> minRect( contours.size() );
    ...
    cout << "max size: " << max_size << "max size of prev: " << prev_dist_rect << endl;
    ...
    for ( int j = 0; j < 4; j++ )
    {   
        line(projected_image, prev_RectPoints.at(j), prev_RectPoints.at((j+1)%4), color);
    } 

	cv::imshow("drawing", projected_image);
    merged_cloud.clear();
